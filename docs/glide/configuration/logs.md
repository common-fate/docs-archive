...
Glide sends all logs to [Amazon CloudWatch](https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/WhatIsCloudWatchLogs.html). Glide uses a JSON structured log format. An example of a log message is shown below.
...
The `userId` field corresponds to an authenticated user in Glide. The `reqId` field is populated from the AWS Lambda request ID.
