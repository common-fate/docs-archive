...
To set up Okta to sync users and groups with Glide we will need to create an access token to communicate with Okta's API.
...
In the meantime, there is a workaround available to get a shortcut to Glide on team members’ Okta dashboards.

1. Enable the “Do not display application icon to users” setting in the Glide SAML application.
...
3. Assign the Bookmark App to the same groups that the Glide SAML app is assigned to.
