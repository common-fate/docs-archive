...
:::info
Glide user and group sync requires some credentials to be configured in your identity provider account. Glide uses 2LO authentication to read users and groups from your directory and sync them to an internal database every 5 minutes.
:::
...
To set up Azure to sync users and groups with Glide we will need to create an access token to communicate with Azure's Graph API.
