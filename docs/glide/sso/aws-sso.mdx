...
To set up AWS IAM Identity Center as your identity provider for Glide you will need to create an IAM role in the management account of your AWS organization that has a trust relationship with the Glide deployment.
...
Ensure that you add the following tag to the role so that the Glide lambda can assume it.
...
Add the Glide **Dashboard URL** from the command output to the **Application start URL** field.

Scroll down to the **Application metadata** section.

Add the Glide **SAML SSO URL (ACS URL)** from the command output to the **Application ACS URL** field.

Add the Glide **Audience URI** from the command output to the **Application SAML audience** field.
...
Finally you will need to create or use an existing group in AWS IAM Identity Center as an administrator group for Glide. Members of this group will have access to the admin dashboard of the application.
Enter the ID of the group in AWS IAM Identity Center which represents the administrators of Glide.
...
You should follow the instructions to update you Glide deployment.
