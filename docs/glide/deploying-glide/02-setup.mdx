...
To manage your Glide deployment, start by installing `gdeploy`.

`gdeploy` is a command-line interface (CLI) tool for creating and managing Glide deployments. It provides commands for creating and updating deployments, and also for operational tasks like viewing logs and backing up or restoring data.
...
To start, create a new folder on your computer and open a terminal window in it; this folder will hold configuration files for your Glide deployment. You can give this folder any name — we suggest calling it `glide-deployment`. In the terminal window, run the following command:
...
git init
git add -A
git commit -m "initial Glide deployment configuration"
```
