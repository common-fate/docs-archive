...
Glide automatically updates the available accounts when OUs are used in Access Rules.
...
When new accounts are added to the Production OU, they will automatically be made available for users to request access to under the same Access Rule. Glide syncs the available accounts every 5 minutes.

## The Root OU

The Root OU is the top level of your organization. In Glide the Root OU is shown as 'Root' when selecting an OU for an Access Rule. It's the parent of all other OUs, and is the only OU that can have accounts directly attached to it. All other OUs must be a child of another OU. Selecting the Root OU in Glide will grant access to every account in your organization: this should be reserved for sandbox accounts, privileged users or for break-glass access.
...
You'll see three fields: Accounts, Organizational Units and Permission Sets. Glide will show you a preview of which accounts will be available for selection by Glide end users. You can also add individual accounts and OUs at the same time.
...
1. Archive the Access Rule in your Glide deployment
