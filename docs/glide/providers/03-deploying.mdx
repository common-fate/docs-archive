...
:::info
To deploy a PDK Provider, you'll need to have [installed the `gdeploy` CLI](./setup), and have valid AWS credentials in your terminal environment.
:::
...
3. An AWS IAM role which allows Glide to invoke the Lambda, called the **Invoke Role**.
...
4. Registers the Handler with Glide.
...
<details>
  <summary>Example output for the `gdeploy oss provider deploy` commnad</summary>
  <div>
    <CodeBlock className="language-bash">
    {`❯ gdeploy oss provider deploy --common-fate-aws-account 123456789012
...
```
gdeploy oss provider destroy --handler-id <handler-id> --target-group-id <target-group-id> --delete-cloudformation-stack
```
...
Running `gdeploy oss provider deploy` walks through the deployment process interactively. If you prefer, you can run commands individually to deploy a Provider.
...
```
gdeploy oss provider bootstrap --id <provider-id> --bootstrap-bucket=DEPLOYMENT_BUCKET
```
...
```
gdeploy oss targetgroup create --id <enter_a_unique_targetgroup_identifier> --schema-from <provider-id>
```
...
```
gdeploy oss handler register --id <enter_a_unique_handler_identifier> --aws-region <your_aws_region> --aws-account <your_aws_accound_id>
```
...
```
gdeploy oss targetgroup link --target-group <target_group_id> --handler <handler_id> --kind <kind_name>
```
...
```
gdeploy oss provider generate cloudformation-create
```

Alternatively, you can prefill these fields in the following command:

```
gdeploy oss cloudformation generate cloudformation-create --provider-id <provider_id> --handler-id <handler_id> --region <aws_region>
```
...
```
gdeploy oss handler validate --id <handler_id> --aws-region <region> --runtime aws-lambda
```
