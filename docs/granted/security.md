...
:::note
The process below will use `v0.3.0` as the version of Granted. Ensure that you change references to `v0.3.0` to the version of Granted you wish to verify when following this process.
:::
...
1. Download the Granted release artifact you wish to verify (we will use the Linux `x86_64` version as an example):
   ...
   ```
   curl -OL releases.commonfate.io/granted/v0.3.0/granted_0.3.0_linux_x86_64.tar.gz
   ```

2. Download the checksums for the release:
   ```bash
   curl -OL releases.commonfate.io/granted/v0.3.0/checksums.txt
   ```
3. Download the signature file:

   ```bash
   curl -OL releases.commonfate.io/granted/v0.3.0/checksums.txt.sig
...
4. Verify the integrity of the release artifact:
   ...
   ```
   granted_0.3.0_linux_x86_64.tar.gz: OK
   ```
