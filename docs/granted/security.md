...
:::note
The process below will use `v0.1.14` as the version of Granted. Ensure that you change references to `v0.1.14` to the version of Granted you wish to verify when following this process.
:::
...
1. Download the Granted release artifact you wish to verify (we will use the Linux `x86_64` version as an example):
   ...
   ```
   curl -OL releases.commonfate.io/granted/v0.1.14/granted_0.1.14_linux_x86_64.tar.gz
   ```

2. Download the checksums for the release:
   ```bash
   curl -OL releases.commonfate.io/granted/v0.1.14/checksums.txt
   ```
...
4. Verify the integrity of the release artifact:
   ...
   ```
   granted_0.1.14_linux_x86_64.tar.gz: OK
   ```
