...
:::note
The process below will use `v0.4.3` as the version of `gdeploy`. Ensure that you change references to `v0.4.3` to the version of Granted Approvals you wish to verify when following this process.
:::
...
1. Download the Granted Approvals release artifact you wish to verify (we will use the Linux `x86_64` version as an example):
   ...
   ```
   curl -OL releases.commonfate.io/gdeploy/v0.4.3/gdeploy_0.4.3_linux_x86_64.tar.gz
   ```

2. Download the checksums for the release:

   ```
   curl -OL releases.commonfate.io/gdeploy/v0.4.3/checksums.txt
   ```

3. Download the signature file:

   ```
   curl -OL releases.commonfate.io/gdeploy/v0.4.3/checksums.txt.sig
   ```
...
4. Verify the integrity of the release artifact:
   ...
   ```
   gdeploy_0.4.3_linux_x86_64.tar.gz: OK
   ```
