...
To manage our Common Fate deployment, we'll start by installing `gdeploy`.
...
To start, we're going to create a folder to hold configuration files for your Common Fate deployment. Create a new folder on your computer and open a terminal window in it. You can give this folder any name — we suggest calling it `granted-deployment`. In the terminal window, run the following command:
...
The `granted-deployment.yml` file contains all of the configuration for your Common Fate deployment. Have a look at the file by running `cat granted-deployment.yml`:
