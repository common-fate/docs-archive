...
<details>
  <summary>Example output for the `gdeploy provider deploy` commnad</summary>
  <div>
    <CodeBlock className="language-bash">
    {`❯ gdeploy provider deploy --common-fate-aws-account 123456789012
...
```
gdeploy provider destroy --handler-id <handler-id> --target-group-id <target-group-id> --delete-cloudformation-stack
```
...
Running `gdeploy provider deploy` walks through the deployment process interactively. If you prefer, you can run commands individually to deploy a Provider.
...
```
gdeploy provider bootstrap --id <provider-id> --bootstrap-bucket=DEPLOYMENT_BUCKET
```
...
```
gdeploy provider generate cloudformation-create
```
