...
## Removing a deployed Provider
- The following command will delete the Target Group and Handler
To uninstall the provider and its associated items, run the following command:
- The `--delete-cloudformation-stack` tag will also delete the cloud formation stack of the handler you had deployed.
...
To delete a Target Group run the following command:
```
cf targetgroup delete
```

## Removing a Handler
To delete a handler run the following command:
- Note this will not destroy the cloud formation stack.
```
cf handler delete
```

## Un-Linking a Target Group and Handler
To Un-Link a handler and target group run the following command:
```
cf targetgroup unlink --kind={Kind}
```